  ...
  return (
    <div className="bg-white rounded-xl border shadow-sm overflow-hidden hidden md:block">
      <Table className="min-w-[800px]">
        ...
        <TableBody>
          {loading ? (
            <TableRow><TableCell colSpan={4} className="py-10 text-center text-gray-500">Memuat data...</TableCell></TableRow>
          ) : data.length === 0 ? (
            <TableRow><TableCell colSpan={4} className="py-10 text-center text-gray-400 italic">Data kosong.</TableCell></TableRow>
          ) : (
            data.map((item, idx) => (
              <TableRow key={item.id} className="hover:bg-indigo-50/30 transition-colors">
                ...
                <TableCell>
                   <span className="font-bold text-gray-800 bg-gray-100 px-2.5 py-1 rounded text-sm border border-gray-200">
                     {item.nama}
                   </span>
                </TableCell>
                <TableCell className="text-gray-700 font-medium text-sm">{item.periodeNama}</TableCell>
                <TableCell>
